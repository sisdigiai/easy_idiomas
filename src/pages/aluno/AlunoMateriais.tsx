...
import { getStudentMaterials } from "@/lib/supabase/queries";
      ...
      <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="font-body-lg text-on-surface-variant">Acesse PDFs, Ã¡udios e recursos das suas turmas.</p>
          </div>
          ...
          {materials.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {materials.map((mat, i) => (
                <div key={i} className="flex flex-col border border-outline-variant/40 rounded-2xl bg-surface-container-lowest overflow-hidden group hover:border-primary/40 transition-colors">
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <p className="font-label-sm text-primary mb-2 mt-1">{mat.class?.name || "Geral"}</p>
                    ...
                    {isSafeExternalUrl(mat.url) ? (
                      <a
                        ...
                        className="inline-flex items-center justify-center w-full rounded-xl border border-outline-variant px-4 py-2 hover:bg-primary/5 transition-colors"
                      >
                        <span className="material-symbols-outlined mr-2">download</span> Acessar
                      </a>
                    ) : (
                      <Button variant="outline" className="w-full rounded-xl" disabled title="Link indisponível">
                        <span className="material-symbols-outlined mr-2">link_off</span> Link indisponível
                      </Button>
                    )}
                  </div>
                </div>
             ...
             <div className="text-center py-12">
               ...
               <p className="text-on-surface-variant font-label-md">Nenhum material encontrado para suas turmas.</p>
             </div>
          )}
        </div>
      </div>
